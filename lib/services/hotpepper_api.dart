...
class HotPepperApi {
  ...
  /// [facilityFilters]: enabled keys → `key=1` (parking, private_room, wifi, …)
  Future<List<Shop>> searchShops({
    ...
    String? genre,
    Map<String, bool> facilityFilters = const {},
  }) async {
    ...
    for (final entry in facilityFilters.entries) {
      if (entry.value) {
        queryParameters[entry.key] = '1';
      }
    }
